...
export default {
    ...
    actions: {
        async getRegisterData({ commit, dispatch }, payload) {
            const APIkey = "AIzaSyCQaTnt70VBqrQHu_l_N85Ut7GI-4D0ImA"
            ...
            try {
                ...
                commit("setToken", {
                    ...
                    expiresIn: new Date().getTime + Number.parseInt(data.expiresIn) * 100
                })
        ...
        async addNewUser({ commit, state }, payload) {
            try {
                const { data } = await axios.post(`https://clothes-project-99880-default-rtdb.firebaseio.com/user.json?auth=${state.token}`, payload)
                commit("setUserLogin", {
                    userData: payload,
                    loginStatus: true
                })
        ...
        async getUser({ commit }, payload) {
            try {
                const { data } = await axios.get(`https://clothes-project-99880-default-rtdb.firebaseio.com/user.json`);
                for (let key in data) {
